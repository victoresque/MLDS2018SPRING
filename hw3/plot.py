...
if __name__ == '__main__':
    ...
    plt.plot([i*iter_per_epoch for i in range(1, n_epoch+1)], epoch_loss_d, 'r', label='loss_D')
    plt.plot([i*iter_per_epoch for i in range(1, n_epoch+1)], epoch_loss_g, 'b', label='loss_G')
    plt.xlim((0, total_iter))
    plt.ylim((0, 8))
    plt.title('CGAN (no emb)')
